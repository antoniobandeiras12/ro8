...
import { InsertUser, users, relatorios, InsertRelatorio } from "../drizzle/schema";
...
// RSO (Relatório de Serviço Operacional) queries
...
export async function getRelatorios() {
  ...
  try {
    const result = await db.select().from(relatorios).orderBy((t) => t.createdAt);
    console.log("[Database] getRelatorios result:", result.length, "relatórios encontrados");
    return result;
  } catch (error) {
    console.error("[Database] Error in getRelatorios:", error);
    throw error;
  }
}
